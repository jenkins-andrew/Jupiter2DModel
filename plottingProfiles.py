...
# Creating grid
xtest = np.arange(-30, 31, 1)
ytest = xtest
xtest, ytest = np.meshgrid(xtest, ytest)

# Masking a circle of radius 20 R_J
mask = (np.sqrt(xtest ** 2 + ytest ** 2) < 6) | (np.sqrt(xtest ** 2 + ytest ** 2) > 30)
...
plt.title('Alfven Velocity', fontsize=18, wrap=True)
plt.xlabel('x $(R_J)$', fontsize=18)
plt.ylabel('y $(R_J)$', fontsize=18)
plt.xticks(size=18)
plt.yticks(size=18)
...
clb = plt.colorbar(heatmap)
clb.ax.set_title(r'(kms$^{-1}$)', fontsize=18)
plt.title('Corotation Velocity', fontsize=18, wrap=True)
plt.xlabel('x $(R_J)$', fontsize=18)
plt.ylabel('y $(R_J)$', fontsize=18)
plt.xticks(size=18)
plt.yticks(size=18)
